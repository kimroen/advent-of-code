...
class TracerTest < Minitest::Test
  ...
  def test_it_can_walk_multiple_steps
    ...
    assert_equal({ x: 0, y: 5 }, tracer.current_coordinates)
  end

  def test_it_can_move_north
    tracer = Tracer.new
    tracer.walk_one_block
    assert_equal({ x: 0, y: 1 }, tracer.current_coordinates)
  end
  ...
  def test_it_can_move_west
    ...
    tracer.apply_heading(:left)
    tracer.walk_one_block
    assert_equal({ x: -1, y: 0 }, tracer.current_coordinates)
  end
  ...
  def test_it_can_move_steps
    tracer = Tracer.new
    tracer.move(2)
    assert_equal({ x: 0, y: 2 }, tracer.current_coordinates)
  ...
  def test_it_can_move_rotation_left
    tracer = Tracer.new
    tracer.move(:left)
    assert_equal({ x: 0, y: 0 }, tracer.current_coordinates)
  ...
  def test_it_exposes_its_current_coordinates
    ...
    assert_equal expected_coordinates, tracer.current_coordinates
  end
end
